...
- 🌐 BROWSER MODE: Playwright fallback for sites requiring real behavior
- 🍪 AUTO COOKIES: Pulls fresh session cookies from Redis automatically
...
- 🧩 CAPTCHA SOLVING: Automatic CAPSOLVER integration (reCAPTCHA, Turnstile, Cloudflare, AWS WAF)
- 🧠 COGNITIVE ENGINE (2026): LLM parsing, vision verification, smart query mutation
...
try:
    ...
    logger.warning("⚠️ Cookie store not available")
...
try:
    ...
    logger.warning("⚠️ Browser mode not available")
...
try:
    ...
    logger.info("🕵️ Stealth mode available (curl_cffi)")
except ImportError:
    STEALTH_AVAILABLE = False
    logger.warning("⚠️ curl_cffi not available, using httpx (less stealthy)")
...
try:
    ...
    logger.warning("⚠️ OpenAI not available - cognitive features disabled")
...
class CircuitBreaker:
    ...
    def record_failure(self) -> None:
        ...
        if self.failure_count >= self.failure_threshold:
            logger.warning(f"🔴 Circuit breaker: OPEN (failed {self.failure_count} times)")
...
class BaseScraper(ABC):
    ...
    Features:
    - 🕵️ STEALTH: Mimics Chrome TLS fingerprint via curl_cffi
    - 🔄 RESILIENCE: Auto-retries on network errors and 429s
    - ⚡ ADAPTIVE: Reads rate limit headers and adjusts speed
    - 🔌 CIRCUIT BREAKER: Pauses after 10 consecutive failures
    - 📼 FLIGHT RECORDER: Logs failed requests for replay
    - 🧩 CAPTCHA SOLVING: Automatic CAPSOLVER integration (reCAPTCHA, Turnstile, Cloudflare, AWS WAF)
    ...
    def __init__(
        ...
        if self.proxy:
            logger.info(f"🌐 Proxy configured: {self.proxy.split('@')[-1] if '@' in self.proxy else 'custom'}")
        
        if self.browser_mode:
            logger.info("🌐 Browser mode enabled (Playwright)")
    ...
    async def _load_cookies_from_redis(self) -> None:
        ...
        try:
            ...
            if cookie_header:
                ...
                self.logger.info(f"🍪 Loaded cookies for {self.platform} from Redis")
            else:
            ...
            if self.auto_load_cookies and self.platform:
                try:
                    ...
                    if cookies:
                        ...
                        self.logger.info(f"🍪 Loaded {len(cookies)} cookies into browser for {self.platform}")
                except Exception as e:
                    self.logger.warning(f"⚠️ Failed to load cookies into browser: {e}")
    ...
    async def _get_session(self) -> Union[Any, httpx.AsyncClient]:
        ...
        if self.use_stealth:
            if self._curl_session is None:
                ...
                self.logger.debug(f"🕵️ Stealth mode: impersonating {profile}")
    ...
    def _calculate_delay(self) -> float:
        ...
        if self.rate_limit.is_near_limit:
            ...
            base_delay *= slowdown
            self.logger.debug(f"⚠️ Near rate limit ({self.rate_limit.remaining}/{self.rate_limit.limit}), slowing to {base_delay:.2f}s")
    ...
    async def _sleep(self, seconds: float) -> None:
        ...
        self.logger.debug(f"💤 Sleeping {actual_sleep:.2f}s")
    ...
    def _record_failure(self, url: str, status: int, headers: Dict[str, str], content: str) -> None:
        ...
        self.logger.error(f"📼 Flight Recorder Dump: {json.dumps(snapshot)}")
    ...
    async def _try_solve_captcha_in_browser(self, browser: Any, url: str) -> bool:
        ...
        try:
                    ...
                    try:
                        ...
                        if shot and len(shot) >= 8:
                            ...
                            if coords:
                                ...
                                self.logger.info("🧩 [BROWSER] OpenAI Vision solved recaptcha image")
            ...
            self.logger.info("🧩 [BROWSER] Capsolver token injected (%s)", ct)
            await asyncio.sleep(2)
            return True
        except Exception as e:
            self.logger.warning("⚠️ [BROWSER] CAPTCHA solve failed: %s", e)
            return False
        ...
        try:
            ...
            if os.getenv("ENABLE_BROWSER_WARMUP", "").lower() in ("1", "true"):
                try:
                    ...
                    pass
            self.logger.info(f"🌐 [BROWSER] Navigating to {url}")
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if status_code == 429:
                    ...
                    self.logger.warning(f"⚠️ Rate Limit Hit (429). Sleeping {retry_after}s...")
                ...
                if status_code >= 400:
                    ...
                    if captcha_info and get_captcha_solver().is_available():
                        try:
                            self.logger.warning(f"🧩 CAPTCHA detected ({captcha_info['type']}) - solving...")
                            ...
                            if solution:
                                ...
                                if captcha_info['type'] == 'cloudflare_challenge' and isinstance(solution, dict):
                                    ...
                                    if cookies:
                                        ...
                                        self.logger.debug(f"🍪 Set {len(cookies)} Cloudflare cookies")
                ...
                backoff = min(2 ** attempt, 60)
                self.logger.warning(f"❌ Error: {e}. Retry {attempt + 1}/{self.max_retries}. Waiting {backoff}s")
                await self._sleep(backoff)
        
        # All retries exhausted
        self.failed_requests += 1
        self.circuit.record_failure()
        self.logger.error(f"💀 All {self.max_retries} retries failed for {url}")
    ...
    async def enable_browser_mode(self) -> bool:
        ...
        if not BROWSER_MODE_AVAILABLE:
            self.logger.error("❌ Browser mode not available (Playwright not installed)")
            return False
        
        self.browser_mode = True
        self.logger.info("🌐 Switched to browser mode (Playwright)")
        return True
    ...
    def _setup_ai(self) -> None:
        ...
        if not OPENAI_AVAILABLE:
            self.logger.warning("⚠️ OpenAI not available - cognitive features disabled")
            return
        
        if self._ai_client is None:
            api_key = os.getenv("OPENAI_API_KEY")
            if not api_key:
                self.logger.warning("⚠️ OPENAI_API_KEY not set - cognitive features disabled")
                return
            
            self._ai_client = OpenAI(api_key=api_key)
            self.logger.debug("🧠 Cognitive Engine initialized")
    ...
    async def parse_with_llm(
        ...
        if not self._ai_client:
            self.logger.warning("⚠️ AI client not available - skipping LLM parse")
            return {}
        ...
        if len(html_snippet) > 15000:
            self.logger.debug(f"📝 Truncated HTML from {len(html_snippet)} to 15000 chars")
...
Do not include any explanation or markdown formatting - just the JSON object."""
        ...
        try:
            self.logger.info(f"🧠 AI Parsing HTML for: {schema_description}")
            ...
            self.logger.success(f"✅ AI extracted {len(result)} fields")
            return result
    ...
    async def verify_page_content(
        ...
        for indicator in block_indicators:
            if indicator in page_lower:
                self.logger.warning(f"🛡️ Block indicator detected: {indicator}")
                return False
    ...
    async def _verify_with_vision(
        ...
        try:
            ...
            if not Path(screenshot_path).exists():
                self.logger.warning(f"⚠️ Screenshot not found: {screenshot_path}")
                return False
            
            import base64
            
            with open(screenshot_path, "rb") as img_file:
                self.logger.info("👁️ Using vision model to verify page content...")
...
{{"is_valid": true/false, "reason": "explanation"}}"""
                ...
                if is_valid:
                    self.logger.success(f"✅ Vision verification passed: {reason}")
                else:
                    self.logger.warning(f"🛡️ Vision verification failed: {reason}")
                
                return is_valid
                
        except Exception as e:
            self.logger.error(f"❌ Vision verification error: {e}")
            return False
    ...
    def generate_query_mutations(
        ...
        self.logger.debug(f"🔄 Generated {len(mutations)} query mutations")
        return mutations
    ...
    async def run(self, **kwargs) -> Any:
        ...
        self.logger.info(f"🚀 Starting {self.__class__.__name__} [{mode}]")
        ...
        try:
            ...
            duration = time.time() - start_time
            self.logger.error(f"💀 Failed after {duration:.2f}s: {e}")
            raise
